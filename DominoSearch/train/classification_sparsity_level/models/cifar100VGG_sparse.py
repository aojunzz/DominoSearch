...
__all__ = [
    'VGG','vgg16_sparse', 'vgg16_bn_sparse',
]
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=100, N=2, M=4):
        ...
        self._initialize_conv_NM_uniform(self.N,self.M)
    ...
    def _initialize_conv_NM_uniform(self,N,M):
        for mod in self.modules():
            if isinstance(mod, SparseConv):
                mod.apply_N_M(N,M)
    ...
    def set_datalayout(self,layout):
        for mod in self.modules():
            ...
            #elif isinstance(mod, torch.nn.Linear): TODOs
            #    pass
    def set_newidea_flag(self,flag = False):
        for m in self.modules():
            if isinstance(m, SparseConv):
                self.new_idea = flag
